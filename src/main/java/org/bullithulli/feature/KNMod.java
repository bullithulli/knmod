...
public class KNMod {
    ...
    public KNMod() {
        ...
        forceKNModForStartsWith.addAll(Arrays.asList("$ renpy.show_screen","$ renpy.jump","$ renpy.load","$ui.interact","$ ui.interact","return ", "call", "menu", "renpy.quit", "$ renpy.quit", "renpy.call", "$ renpy.call", "renpy.block", "$ renpy.block", "if ", "else ", "elif ", "label", "show screen ", "((", "$ MainMenu(", "$MainMenu(", "$ renpy.quit(", "$renpy.quit(","$ renpy.full_restart","$renpy.full_restart"));
